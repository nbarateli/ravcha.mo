...
var list = [],
    recipes = [];
var workingList = void 0;
var awesome = void 0;
var chosenIngredients = new Set();
...
function renderMustache(item, templateId) {
    var tmp = document.getElementById(templateId).innerHTML;
    Mustache.parse(tmp);
...
function showModal(e) {
    ...
    var id = target.getElementsByTagName('input')[0].value;
    var modal = document.getElementById('recipe-modal');
    var content = modal.getElementsByClassName('recipe-content')[0];
...
function renderRecipe(rec) {
    var html = renderMustache(rec, 'recipe-template');
...
function removeIngredient(e) {
    ...
    var ingredient = e.target.parentElement.parentElement;
    var id = ingredient.children[0].value;
...
function parseIngredient(item) {
    if (!chosenIngredients.has(item.id)) {
        ...
        var html = renderMustache(item, 'ingredient-template');
        ...
        var compFn = function compFn(a, b) {
            return a.id - b.id;
        };
        workingList.sort(compFn);
        var id = binarySearch(workingList, item, compFn);
...
function autoComplete() {
    var inputs = document.getElementsByClassName('ingredient-n');
    addAwesome(inputs[0]);
}
...
function submit() {
    ...
    var _loop = function _loop(i) {
        ...
        chosenIngredients.forEach(function (el) {
            if (isStrict && recipes[i].ingredients[el] === undefined) {
                recipes[i].flag = true;
            }
            if (recipes[i].ingredients[el] !== undefined) recipes[i].ingredientCount++;
        });
    };
...
function ready() {
    ...
    ajax('database/items.json', function (data) {
        list = JSON.parse(data);
        autoComplete(list);
        // for (let i = 0; i < list.length; i++) parseIngredient(list[i])
    });
    ajax('database/recipes.json', function (data) {
...
function parseData(url, parse, nextStep) {
    ajax(url, function () {
        if (parse instanceof Function) parse(data);
...
if (document.readyState === "complete") ready(); else {
